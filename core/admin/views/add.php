<form class="vg-wrap vg-element vg-ninteen-of-twenty" method="post" action="/admin/shop/edit"
    ...
    <?php
        ...
        foreach($this->blocks as $class => $block) {
            ...
            if(is_int($class)) $class = 'vg-content'; // если забыли создать blockNeddle то кидаем по стандарту в vg-rows
                    ...
                    foreach($this->templateArr as $template => $items) {
                        if(in_array($row, $items)) {
                            if(!@include $_SERVER['DOCUMENT_ROOT'] . $this->formTemplates . $template . '.php') {
                                throw new \core\base\exceptions\RouteException('Не найжен шаблон ' .
    ...
    <?php if($this->data): ?>
        <!-- атрибут input подключается только если в состоянии редактирования -->
